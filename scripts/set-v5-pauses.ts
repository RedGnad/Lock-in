...
function flag(value: string | undefined): boolean {
  if (value !== "true" && value !== "false") throw new Error("Pass exactly: <creation> <joining> <evidence> as true/false");
  return value === "true";
}

const values = process.argv.slice(2);
if (values.length !== 3) throw new Error("Pass exactly three pause booleans");
const [creation, joining, evidence] = values.map(flag);
const rawAddress = process.env.NEXT_PUBLIC_LOCK_IN_ESCROW_ADDRESS?.trim() || "";
if (!isAddress(rawAddress)) throw new Error("Missing V5 escrow address");
const escrow = getAddress(rawAddress);
const rpcUrl = process.env.MONAD_RPC_URL?.trim() || "https://rpc.monad.xyz";
const chain = defineChain({ id: 143, name: "Monad", nativeCurrency: { name: "MON", symbol: "MON", decimals: 18 }, rpcUrls: { default: { http: [rpcUrl] } } });
...
const [chainId, version, owner] = await Promise.all([
  ...
  publicClient.readContract({ address: escrow, abi: [{ type: "function", name: "owner", stateMutability: "view", inputs: [], outputs: [{ name: "", type: "address" }] }] as const, functionName: "owner" }),
]);
...
const operations = [["setCreationPaused", creation], ["setJoiningPaused", joining], ["setEvidencePaused", evidence]] as const;
...
console.log(JSON.stringify({ chainId, escrow, owner, pauses: { creation, joining, evidence }, hashes }, null, 2));
