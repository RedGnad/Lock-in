  ...
  return (
    <section className="create-card" id="create">
      ...
      <div className="form-stage">
        ...
        {step === 1 && <fieldset className="form-field"><legend><b>Set the pace</b><span>{mission.name} · choose a daily target, duration, and crew.</span></legend><div className="field-group"><span className="field-cap">Daily target</span><div className="segmented target-options">{mission.targets.map((item) => <button type="button" className={dailyTarget === item.value ? "active" : ""} aria-pressed={dailyTarget === item.value} onClick={() => setDailyTarget(item.value)} key={item.value}>{item.label}</button>)}</div></div><div className="field-group"><span className="field-cap">Schedule</span><div className="segmented schedule-options">{PACT_TEMPLATES.map((item) => <button type="button" className={durationDays === item.durationDays ? "active" : ""} aria-pressed={durationDays === item.durationDays} onClick={() => setDurationDays(item.durationDays)} key={item.id}>{item.durationDays}<small>DAYS · {item.requiredCompletions} WINS</small></button>)}</div></div><div className="field-group"><span className="field-cap">Crew</span><div className="segmented crew-options" aria-label="Maximum crew size">{[2, 4, 8].map((size) => <button type="button" className={maxParticipants === size ? "active" : ""} aria-pressed={maxParticipants === size} onClick={() => setMaxParticipants(size)} key={size}>{size}<small>PLAYERS MAX</small></button>)}</div></div></fieldset>}
