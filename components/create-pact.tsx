...
export function CreatePact() {
  ...
  async function create() {
    ...
    try {
      ...
      const receipt = await writeWithGas({
        ...
        args: [amount, dailyTarget, template.durationDays, template.requiredCompletions, 2, maxParticipants, startsAt, mission.type, access],
      }, "create");
      const logs = parseEventLogs({ abi: lockInAbi, eventName: "PactCreated", logs: receipt.logs });
      const id = logs[0]?.args.pactId;
  ...
  if (duolingo) {
    return <DuolingoCreate onCreated={(pactId) => router.push(`/duolingo?lock=${pactId}`)} onBackToMission={() => setDuolingo(false)} />;
  }
  ...
  return (
    <section className="create-card">
      ...
      <div className="form-stage">
        {step === 0 && <fieldset className="form-field"><legend><b>Choose your mission</b></legend><div className="mission-options" role="group" aria-label="Mission">{MISSIONS.map((item) => { const selected = missionChosen && missionId === item.id; return <button type="button" className={selected ? "active" : ""} aria-pressed={selected} onClick={() => chooseMission(item.id)} key={item.id}><strong>{item.name}</strong><span>{item.description}</span></button>; })}<button type="button" className="mission-link" onClick={() => setDuolingo(true)}><strong>Duolingo XP <span className="mission-beta">BETA</span></strong><span>Your XP progress on Duolingo.</span></button></div></fieldset>}
        {step === 1 && <fieldset className="form-field"><legend><b>Set the pace</b><span>{mission.name} · choose a run target, schedule, and crew.</span></legend><div className="field-group"><span className="field-cap">Run target</span><div className="segmented target-options">{mission.targets.map((item) => <button type="button" className={dailyTarget === item.value ? "active" : ""} aria-pressed={dailyTarget === item.value} onClick={() => setDailyTarget(item.value)} key={item.value}>{item.label}</button>)}</div></div><div className="field-group"><span className="field-cap">Schedule</span><div className="segmented schedule-options">{PACT_TEMPLATES.map((item) => <button type="button" className={durationDays === item.durationDays ? "active" : ""} aria-pressed={durationDays === item.durationDays} onClick={() => setDurationDays(item.durationDays)} key={item.id}>{item.requiredCompletions}<small>RUNS IN {item.durationDays} DAYS</small></button>)}</div></div><div className="field-group"><span className="field-cap">Crew</span><div className="segmented crew-options" aria-label="Maximum crew size">{[2, 4, 8].map((size) => <button type="button" className={maxParticipants === size ? "active" : ""} aria-pressed={maxParticipants === size} onClick={() => setMaxParticipants(size)} key={size}>{size}<small>PLAYERS MAX</small></button>)}</div></div></fieldset>}
      ...
      <ActionDialog open={reviewOpen} title="Create this lock?" eyebrow="Transaction review" confirmLabel={allowance < amount ? `Approve ${stakeInput} ${symbol}` : `Stake ${stakeInput} ${symbol} & create`} busy={busy} onClose={() => setReviewOpen(false)} onConfirm={create}>
        ...
        <p>Each day needs a GPS run from your connected Strava account, inside that day of the lock. Manual, treadmill, flagged and implausible runs are rejected, and one run can only ever count once.</p>
