...
export function PactDashboard({ id }: { id: string }) {
  ...
  // The card's exact state. Only "strava_not_connected" means the database really has nothing: an expired
  // wallet session must never be rendered as "connect Strava", because following that advice re-authorises
  // a live grant and rotates the very refresh token the check-in was about to use.
  const [stravaView, setStravaView] = useState<StravaView["kind"]>("loading");
