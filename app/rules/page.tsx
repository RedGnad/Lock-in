export default function RulesPage() {
  return (
    <main className="legal-page">
      ...
      <section><h2>Two missions</h2><p>A Strava lock requires a challenge-named GPS Run that reaches the distance target. Manual, trainer, Strava-flagged, missing-GPS, and implausible motion records are rejected. A Duolingo lock requires a profile bio code that binds the profile to the wallet, then counts only new cumulative XP above the baseline accepted when that wallet stakes. One Duolingo profile cannot back two wallets in the same lock, and the same XP range cannot be reused across days or locks.</p></section>
      ...
      <section><h2>Lock schedule</h2><p>A lock lasts 3 to 30 fixed 24-hour days and publishes its required wins before anyone joins. Registration closes at the displayed start. Only one verified completion can count per wallet and lock day. A lock must reach its published minimum crew before completions count.</p></section>
      <section><h2>Funds</h2><p>Every participant deposits the same amount, from 0.1 to 1 USDC per lock. Monad gas is separate and is never refunded by the lock. Lock In charges no protocol fee in this beta.</p></section>
      <section><h2>Payouts</h2><p>If anyone reaches the target, finishers recover their stakes and split non-finishers&apos; stakes equally. If everyone or nobody finishes, each participant recovers their own stake. Cancelled and underfilled locks are refundable. Settlement and claims are permissionless onchain transactions.</p></section>
      <section><h2>Controls and risk</h2><p>The creator may cancel before the start. The contract owner may pause creation, joining, or evidence and may cancel an unsettled lock only into refunds; pauses never block settlement or claims. The contract is unaudited. Smart-contract, wallet, RPC, token, chain, provider, signer, and key failures can cause delay or loss.</p></section>
