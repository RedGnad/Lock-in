...
export async function generateMetadata({ params }: LockPageProps): Promise<Metadata> {
  ...
  try {
    ...
    const mission = missionByType(lock[10]);
    const title = `${mission.name}: ${lock[8]} in ${lock[7]} days — Lock In`;
    const description = `Join Lock #${id}: ${formatMissionTarget(lock[10], lock[3])}, ${lock[8]} wins in ${lock[7]} days, with ${Number(lock[2]) / 1_000_000} USDC staked per player.`;
    const url = `/lock/${id}`;
