...
### Update 2: executed version is a clean 6.0.0 (not -ai)

`fetchStatusUrl(fa8968844e)`: `statusV2 = PROOF_SUBMITTED` (not AI_PROOF_SUBMITTED), `providerVersionString = "6.0.0"`, `providerVersion = {major:6,minor:0,patch:0,prereleaseTag:null}`. So the session executed exactly the deterministic 6.0.0 provider, no `-ai.*` prerelease. Combined with verifyProof/TEE passing, this strongly indicates isAiProof is a portal-orchestration flag, not the AI-provider path. Still to confirm on the dashboard: `verificationType == WITNESS`.
