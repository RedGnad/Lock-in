 ...
 *
 * LockInDuolingoEscrow moves real USDC, so its backend opens to a named set of wallets and no one else,
 * under DUOLINGO_ESCROW_ALLOWED_WALLETS. This is separate from DUOLINGO_PREVIEW_ALLOWED_WALLETS (the Live
 * Proof Beta, no stake) and from CANARY_ALLOWED_WALLETS (the Strava escrow): a wallet enabled for one
 * product is not thereby enabled for the money one.
 *
