...
export function issueProofSession(payload: Omit<ProofSession, "exp">): string {
  const encoded = Buffer.from(JSON.stringify({ ...payload, exp: Date.now() + 20 * 60_000 })).toString("base64url");
  return `${encoded}.${signature(encoded)}`;
}

export function verifyProofSession(token: string): ProofSession {
  ...
  const value = JSON.parse(Buffer.from(encoded, "base64url").toString("utf8")) as Partial<ProofSession>;
  if (
